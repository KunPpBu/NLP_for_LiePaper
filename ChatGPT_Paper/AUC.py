...
pred_nltk = le.fit_transform(df['nltk_sentiment_summary'])
label_nltk = le.fit_transform(df['nltk_sentiment_fulltext'])
...
plt.plot(fpr,tpr,label="NLTK, auc="+str(round(auc,3)))
plt.legend(loc=4)
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Sentiment analysis AUC with NLTK \n accuracy = '+str(round(acc,3)))
plt.show()